...
const { games, guideMeta, entityMeta = [], termVisuals = {} } = context.GameOriContent;
...
function entityImagePath(entity) {
  const visual = entityVisualFor(entity);
  return visual && visual.image ? visual.image : gameImagePath(entity.game);
}
...
Object.entries(games).forEach(([id, game]) => {
  ...
  const entities = entityMeta.filter((entity) => entity.game === id);
        ...
        schema: [
          {
            ...
            hasPart: guides.map((guide) => ({
              ...
              entities.map((entity) => ({
                "@type": "DefinedTerm",
                name: entity.name,
                alternateName: entity.enName,
                url: absoluteUrl(`games/${entity.game}/terms/${entity.slug}/`),
              })),
            ),
          },
          breadcrumbSchema([
  ...
  if (entities.length) {
    const databasePath = path.join("games", id, "database");
    sitemapUrls.push(`${databasePath}/`);
    writePage(
      databasePath,
      routeHtml(
        `${game.name} 图文资料库 | GameOri`,
        `${game.cn} ${game.name} 图文资料库，集中整理术语、资源、系统和地图点，并关联攻略文章。`,
        {
          ...
          schema: [
            {
              "@context": "https://schema.org",
              "@type": "CollectionPage",
              name: `${game.name} 图文资料库`,
              description: `${game.cn} ${game.name} 图文资料库，集中整理术语、资源、系统和地图点，并关联攻略文章。`,
              url: absoluteUrl(`${databasePath}/`),
              isPartOf: {
                "@type": "CollectionPage",
                name: `${game.name} 攻略专题`,
            ...
            breadcrumbSchema([
              { name: "GameOri", path: "index.html" },
              { name: `${game.name} 攻略专题`, path: `games/${id}/` },
              { name: `${game.name} 图文资料库`, path: `${databasePath}/` },
            ]),
          ],
        },
      ),
...
entityMeta.forEach((entity) => {
  ...
  const relativePath = path.join("games", entity.game, "terms", entity.slug);
  sitemapUrls.push(`${relativePath}/`);
  const visual = entityVisualFor(entity);
  writePage(
    relativePath,
    routeHtml(`${entity.name} | ${game.name} 图文词条`, `${game.cn} ${entity.name}：${entity.description}`, {
      canonicalPath: `${relativePath}/`,
      imagePath: entityImagePath(entity),
      baseHref: "../../../../",
      schema: [
        {
          "@context": "https://schema.org",
          ...
          url: absoluteUrl(`${relativePath}/`),
          image: absoluteUrl(entityImagePath(entity)),
          inDefinedTermSet: {
            "@type": "DefinedTermSet",
            name: `${game.name} 图文资料库`,
            url: absoluteUrl(`games/${entity.game}/database/`),
          },
          isPartOf: {
            "@type": "CollectionPage",
            name: `${game.name} 攻略专题`,
            url: absoluteUrl(`games/${entity.game}/`),
          },
          subjectOf: entity.relatedGuides.map((guideId) => {
            const guide = guideMeta.find((item) => item.id === guideId);
            return {
              "@type": "Article",
              headline: guide ? guide.title : guideId,
              url: absoluteUrl(`guides/${guideId}/`),
            };
          }),
          sameAs: visual && visual.source ? visual.source : undefined,
        },
        breadcrumbSchema([
          { name: "GameOri", path: "index.html" },
          { name: `${game.name} 攻略专题`, path: `games/${entity.game}/` },
          { name: `${game.name} 图文资料库`, path: `games/${entity.game}/database/` },
          { name: entity.name, path: `${relativePath}/` },
        ]),
      ],
    }),
  );
...
console.log(`Generated ${Object.keys(games).length} game pages, ${guideMeta.length} guide pages, and ${entityMeta.length} term pages.`);
