...
const copy = {
  zh: {
    ...
    databaseEyebrow: "资料库",
    databaseTitle: "图文词条与系统资料",
    databaseHint: "像查 Wiki 一样查术语、资源、系统和地图点，再回到攻略路线执行。",
    databaseOpen: "进入资料库",
    entityCount: "个词条",
    entityVerified: "已有专图",
    entityNeedsImage: "待补专图",
    entityRelated: "关联攻略",
    entityBackDatabase: "返回资料库",
    entityWhere: "出现位置",
    entityUse: "怎么用",
    entitySource: "资料来源",
    entityTypes: {
    ...
    articleFaq: "常见问题",
    articleDecision: "决策表",
    articleVisualGuide: "图文速读",
    articleVisualLead: "先看画面里的场景、资源和交互，再按下面路线执行。",
    articleVisualLook: "看图先找",
    articleVisualRoute: "路线截图",
    articleVisualResources: "资源图卡",
    articleVisualSource: "图片来源",
    articleVisualMissing: "待补专图",
    articleVisualFallback: "还没有找到可复核的术语截图，欢迎投稿补充。",
  ...
  en: {
    ...
    databaseTitle: "Visual entries and system notes",
    databaseHint: "Look up terms, resources, systems, and map nodes like a light wiki, then return to the guide route.",
    databaseOpen: "Open database",
    entityCount: "entries",
    entityVerified: "Verified image",
    entityNeedsImage: "Needs image",
    entityRelated: "Related guides",
    entityBackDatabase: "Back to database",
    entityWhere: "Where it appears",
    entityUse: "How to use it",
    ...
    articleVisualGuide: "Visual walkthrough",
    articleVisualLead: "Read the scene, resources, and interaction points first, then follow the route below.",
    articleVisualLook: "Look for",
    articleVisualRoute: "Route shots",
    articleVisualResources: "Resource cards",
    articleVisualSource: "Image source",
    articleVisualMissing: "Image needed",
    articleVisualFallback: "No verified term-specific screenshot yet. Community submissions are welcome.",
...
const { art, games, guideMeta, termDescriptions, termVisuals = {}, entityMeta = [] } = globalThis.GameOriContent;
...
function termVisualFor(guide, resource) {
  const gameVisuals = termVisuals[guide.game] || {};
  const visual = gameVisuals[resource];
  if (!visual) return null;
  return {
    resource,
    image: visual.image,
    source: visual.source,
    sourceLabel: visual.sourceLabel || t("articleVisualSource"),
    caption: visual.caption || resource,
  };
}

function visualSourceMarkup(visual) {
  if (!visual || !visual.source) return "";
  return `<a class="visual-source-link" href="${visual.source}" target="_blank" rel="noopener noreferrer">${t("articleVisualSource")} · ${visual.sourceLabel}</a>`;
}

function visualMediaMarkup(visual, alt) {
  if (!visual || !visual.image) {
    return `<div class="visual-missing-media" aria-label="${t("articleVisualMissing")}"><strong>${t("articleVisualMissing")}</strong><span>${t("articleVisualFallback")}</span></div>`;
  }
  return `<img src="${visual.image}" alt="${alt}" loading="lazy">`;
}

function articleVisualPlan(guide, article, game) {
  const crops = ["50% 28%", "68% 42%", "36% 58%", "76% 64%"];
  const resources = guide.resources.slice(0, 4);
  const cues = resources.map((resource, index) => {
    const visual = termVisualFor(guide, resource);
    return {
      resource,
      position: crops[index % crops.length],
      image: visual ? visual.image : "",
      source: visual ? visual.source : "",
      sourceLabel: visual ? visual.sourceLabel : "",
      caption: visual ? visual.caption : "",
      hasVisual: Boolean(visual && visual.image),
      text:
        index === 0
          ? `进入当前内容前，先在画面或菜单里确认「${resource}」是否已经解锁。`
          : index === 1
            ? `推进过程中重点观察「${resource}」的数量、状态或可用窗口。`
            : `路线稳定后再回头检查「${resource}」，避免为了次要目标打断主线节奏。`,
    };
  });
  const firstVisual = cues.find((cue) => cue.hasVisual) || cues[0] || null;

  return {
    lead: `${game.cn} 的这篇攻略优先使用术语对应的真实图像；没有核实到专图的项目会标记为待补，不再用封面图冒充。`,
    cues,
    firstVisual,
    routePanels: article.phases.map((phase, index) => ({
      ...phase,
      position: crops[index % crops.length],
      visual: cues[index % Math.max(1, cues.length)] || firstVisual,
...
function databasePath(id) {
  return routePath(`games/${id}/database/index.html`);
}

function entityPath(entity) {
  return routePath(`games/${entity.game}/terms/${entity.slug}/index.html`);
}

function entityTypeLabel(type) {
  const labels = t("entityTypes") || {};
  return labels[type] || type;
}

function entityVisualFor(entity) {
  const gameVisuals = termVisuals[entity.game] || {};
  return gameVisuals[entity.name] || null;
}

function entityByRoute(gameId, slug) {
  return entityMeta.find((item) => item.game === gameId && item.slug === slug);
}

function fullEntities() {
  const guides = fullGuides();
  return entityMeta.map((entity) => {
    const game = games[entity.game];
    const visual = entityVisualFor(entity);
    const relatedGuides = (entity.relatedGuides || [])
    ...
    return {
      ...
      visual,
      relatedGuides,
      image: visual && visual.image ? visual.image : "",
      source: visual && visual.source ? visual.source : "",
      sourceLabel: visual && visual.sourceLabel ? visual.sourceLabel : "",
    };
  });
}

function entitiesForGame(gameId) {
  return fullEntities().filter((entity) => entity.game === gameId);
}

function entityById(id) {
  return fullEntities().find((entity) => entity.id === id);
}

function entityForGameTerm(gameId, term) {
  return fullEntities().find((entity) => entity.game === gameId && entity.name === term);
}

function entityTitleMarkup(gameId, term) {
  const entity = entityForGameTerm(gameId, term);
  if (!entity) return `<strong>${term}</strong>`;
  return `<strong><a class="entity-inline-link" href="${entityPath(entity)}" data-entity="${entity.id}">${term}</a></strong>`;
}

function hasSelectedGamePage() {
  return Boolean(state.selectedGame && games[state.selectedGame] && !state.selectedGuide && !state.selectedDatabaseGame && !state.selectedEntity);
}
...
function hasSelectedDatabasePage() {
  return Boolean(state.selectedDatabaseGame && games[state.selectedDatabaseGame] && !state.selectedGuide && !state.selectedEntity);
}

function hasSelectedEntityPage() {
  return Boolean(state.selectedEntity && fullEntities().some((entity) => entity.id === state.selectedEntity));
}

function hasContentPage() {
  return hasSelectedGamePage() || hasSelectedGuidePage() || hasSelectedDatabasePage() || hasSelectedEntityPage();
}

function setPageMode() {
  const gamePage = hasSelectedGamePage();
  const guidePage = hasSelectedGuidePage();
  const databasePage = hasSelectedDatabasePage();
  const entityPage = hasSelectedEntityPage();
  document.body.classList.toggle("is-topic-page", gamePage || guidePage || databasePage || entityPage);
  document.body.classList.toggle("is-guide-page", guidePage);
  document.body.classList.toggle("is-database-page", databasePage || entityPage);
  $(".command-deck").hidden = gamePage || guidePage || databasePage || entityPage;
  $("#guides").hidden = gamePage || guidePage || databasePage || entityPage;
  $("#games").hidden = !(gamePage || databasePage || entityPage);
...
function parseRoute() {
  ...
  if (gameIndex !== -1 && parts[gameIndex + 1] && parts[gameIndex + 2] === "database") {
    return { type: "database", id: parts[gameIndex + 1] };
  }
  if (gameIndex !== -1 && parts[gameIndex + 1] && parts[gameIndex + 2] === "terms" && parts[gameIndex + 3]) {
    return { type: "entity", game: parts[gameIndex + 1], slug: parts[gameIndex + 3] };
  }
...
function syncRouteToState() {
  ...
  if (route.type === "game" && games[route.id]) {
    ...
    state.selectedDatabaseGame = null;
    state.selectedEntity = null;
    return;
  }
  if (route.type === "database" && games[route.id]) {
    state.selectedGame = route.id;
    state.game = route.id;
    state.selectedGuide = null;
    state.selectedDatabaseGame = route.id;
    state.selectedEntity = null;
    return;
  }
  if (route.type === "entity") {
    const entity = entityByRoute(route.game, route.slug);
    if (entity) {
      state.selectedGame = entity.game;
      state.game = entity.game;
      state.selectedGuide = null;
      state.selectedDatabaseGame = null;
...
function renderGuideTools() {
  ...
  const contentPage = hasContentPage();
...
function filteredGames() {
  ...
  return Object.entries(games).filter(([id, game]) => {
    ...
    if (!query) return true;
    const entities = entitiesForGame(id);
    return [game.name, game.cn, game.studio, game.genre]
      .concat(guides.map((guide) => `${guide.title} ${guide.summary}`))
      .concat(entities.map((entity) => `${entity.name} ${entity.enName} ${entity.description}`))
      .join(" ")
      .toLowerCase()
      .includes(query);
  });
...
function renderGameDetail() {
  ...
  const gameEntities = entitiesForGame(state.selectedGame);
  const verifiedEntities = gameEntities.filter((entity) => entity.status === "verified-image").length;
  const databasePreview = gameEntities.slice(0, 8);
  ...
  detail.innerHTML = `
    <div class="game-page-hero" style="--art: ${game.art}">
      ...
      <div class="deck-metrics game-metrics">
        ...
        <article><strong>${gameEntities.length}</strong><span>${t("entityCount")}</span></article>
      </div>
    </div>
    ...
    <section class="topic-database-strip">
      <div class="topic-database-copy">
        <p class="field-label">${t("databaseEyebrow")}</p>
        <h3>${t("databaseTitle")}</h3>
        <p>${t("databaseHint")}</p>
        <div class="database-mini-metrics">
          <span>${gameEntities.length} ${t("entityCount")}</span>
          <span>${verifiedEntities} ${t("entityVerified")}</span>
        </div>
        <a class="database-link" href="${databasePath(state.selectedGame)}" data-database-game="${state.selectedGame}">${t("databaseOpen")}</a>
      </div>
      <div class="database-preview-grid">
        ${databasePreview
          .map(
            (entity) => `
              <a class="entity-mini-card" href="${entityPath(entity)}" data-entity="${entity.id}">
                <span>${entityTypeLabel(entity.type)}</span>
                <strong>${entity.name}</strong>
                <small>${entity.enName}</small>
              </a>
            `,
          )
          .join("")}
...
function renderDatabaseDetail() {
  const detail = $("#gameDetail");
  const section = $("#games");
  const gameId = state.selectedDatabaseGame;
  ...
  const entities = entitiesForGame(gameId);
  const verified = entities.filter((entity) => entity.status === "verified-image").length;
  const grouped = ["system", "resource", "map", "term", "boss"]
    .map((type) => ({ type, items: entities.filter((entity) => entity.type === type) }))
  ...
  detail.innerHTML = `
    <div class="database-page">
      <nav class="breadcrumbs" aria-label="Breadcrumb">
        ...
        <span>/</span>
        <a href="${gamePath(gameId)}" data-game="${gameId}" class="open-game">${game.name}</a>
        <span>/</span>
        <strong>${t("databaseTitle")}</strong>
      </nav>
      <header class="database-hero" style="--art: ${game.art}">
        <img class="database-hero-image" src="${gameImage(gameId)}" alt="${game.cn} ${game.name} 资料库封面">
        <a class="text-action back-game" href="${gamePath(gameId)}" data-game="${gameId}">${t("backTopic")}</a>
        <p class="eyebrow">${t("databaseEyebrow")}</p>
        <h1>${game.name} ${t("databaseTitle")}</h1>
        <p>${t("databaseHint")}</p>
        <div class="deck-metrics game-metrics">
          <article><strong>${entities.length}</strong><span>${t("entityCount")}</span></article>
          <article><strong>${verified}</strong><span>${t("entityVerified")}</span></article>
          <article><strong>${entities.length - verified}</strong><span>${t("entityNeedsImage")}</span></article>
        </div>
      </header>
      <div class="database-layout">
        <aside class="database-roadmap">
          <p class="field-label">${t("featuredSeries")}</p>
          ${grouped.map((group) => `<a href="#entity-${group.type}"><strong>${entityTypeLabel(group.type)}</strong><span>${group.items.length}</span></a>`).join("")}
          <p class="field-label">${t("detailSources")}</p>
          <div class="source-list">
            ${game.sources.map((source) => `<a href="${source.url}" target="_blank" rel="noopener noreferrer">${source.label}</a>`).join("")}
          </div>
        ...
        <section class="database-content">
          ${grouped
            .map(
              (group) => `
                <section class="entity-group" id="entity-${group.type}">
                  <div class="entity-group-head">
                    <div>
                      <p class="field-label">${entityTypeLabel(group.type)}</p>
                      <h2>${entityTypeLabel(group.type)} ${t("databaseTitle")}</h2>
                    </div>
                    <span>${group.items.length} ${t("entityCount")}</span>
                  </div>
                  <div class="entity-grid">
                    ${group.items
                      .map(
                        (entity) => `
                          <a class="entity-card" href="${entityPath(entity)}" data-entity="${entity.id}">
                            <figure class="entity-card-media ${entity.image ? "" : "is-missing-visual"}">
                              ${visualMediaMarkup(entity, `${game.cn} ${entity.name} 词条图片`)}
                            </figure>
                            <div class="entity-card-copy">
                              ...
                              <em>${entity.relatedGuides.length} ${t("entityRelated")} · ${entity.status === "verified-image" ? t("entityVerified") : t("entityNeedsImage")}</em>
                            </div>
                          </a>
                        `,
                      )
...
function renderEntityDetail() {
  const detail = $("#gameDetail");
  const section = $("#games");
  const entity = entityById(state.selectedEntity);
  if (!entity || !entity.gameData) {
    section.hidden = true;
    detail.innerHTML = "";
    return;
  }

  const game = entity.gameData;
  const relatedGuides = entity.relatedGuides.length
    ? entity.relatedGuides
    : fullGuides()
        .filter((guide) => guide.game === entity.game)
        .slice(0, 4);
  const siblings = entitiesForGame(entity.game)
  ...
  detail.innerHTML = `
    <article class="entity-page">
      <nav class="breadcrumbs" aria-label="Breadcrumb">
        <a href="${homePath()}">${t("breadcrumbHome")}</a>
        <span>/</span>
        <a href="${gamePath(entity.game)}" data-game="${entity.game}" class="open-game">${game.name}</a>
        <span>/</span>
        <a href="${databasePath(entity.game)}" data-database-game="${entity.game}" class="database-link">${t("databaseEyebrow")}</a>
        <span>/</span>
        <strong>${entity.name}</strong>
      </nav>
      <header class="entity-hero" style="--art: ${game.art}">
        <figure class="entity-hero-media ${entity.image ? "" : "is-missing-visual"}">
          ${visualMediaMarkup(entity, `${game.cn} ${entity.name} 词条图`)}
        </figure>
        <div class="entity-hero-copy">
          <a class="text-action back-game database-link" href="${databasePath(entity.game)}" data-database-game="${entity.game}">${t("entityBackDatabase")}</a>
          <p class="eyebrow">${entityTypeLabel(entity.type)}</p>
          <h1>${entity.name}</h1>
          <p>${entity.enName} · ${game.name}</p>
          <span class="entity-status">${entity.status === "verified-image" ? t("entityVerified") : t("entityNeedsImage")}</span>
          ${visualSourceMarkup(entity)}
        </div>
      </header>
      <div class="entity-layout">
        <section class="entity-panel entity-definition">
          <p class="field-label">${t("articleTerms")}</p>
          <h2>${entity.description}</h2>
        </section>
        <section class="entity-panel">
          <p class="field-label">${t("entityWhere")}</p>
          <div class="entity-note-list">
            <article>
              <strong>${game.cn}</strong>
              <p>常见于「${entity.relatedGuides.map((guide) => guide.title).slice(0, 3).join(" / ") || game.genre}」相关内容，适合在进入路线前先确认解锁、入口或资源状态。</p>
            </article>
          </div>
        </section>
        <section class="entity-panel">
          <p class="field-label">${t("entityUse")}</p>
          <div class="entity-note-list">
            <article>
              <strong>攻略用法</strong>
              <p>先看图片和名称定位它在游戏里的界面或场景，再打开关联攻略按步骤处理。没有专图的词条会保留待补标记，方便后续投稿补证据。</p>
            </article>
          </div>
        </section>
        <section class="entity-panel">
          <p class="field-label">${t("entityRelated")}</p>
          <div class="entity-related-list">
            ${relatedGuides
              .map(
                (guide) => `
                  <a href="${guidePath(guide.id)}" data-guide="${guide.id}" class="related-guide article-card">
                    <img src="${guide.image}" alt="${game.cn} ${guide.title} 相关攻略图" loading="lazy">
                    <span>${t("types")[guide.type]}</span>
                    <strong>${guide.title}</strong>
                  </a>
                `,
        ...
        <section class="entity-panel">
          <p class="field-label">${t("topicRadar")}</p>
          <div class="entity-sibling-grid">
            ${siblings
              .map(
                (item) => `
                  <a class="entity-mini-card" href="${entityPath(item)}" data-entity="${item.id}">
...
function renderArticleDetail() {
  ...
  const freshness = guideFreshness(guide);
  const visual = articleVisualPlan(guide, article, game);
  const primaryVisualCue = visual.firstVisual || { resource: guide.resources[0], text: guide.summary };
    ...
    <article class="article-shell">
      ...
      <div class="article-layout">
        <aside class="article-toc">
          ...
          <a href="#article-visual">${t("articleVisualGuide")}</a>
        ...
        <div class="article-body">
          ...
          <section id="article-visual" class="article-block article-visual-block">
            <div class="visual-section-head">
              <p class="field-label">${t("articleVisualGuide")}</p>
              <h2>${t("articleVisualLead")}</h2>
              <p>${visual.lead}</p>
            </div>
            <div class="visual-walkthrough">
              <figure class="visual-main-shot ${primaryVisualCue.hasVisual ? "" : "is-missing-visual"}">
                ${visualMediaMarkup(primaryVisualCue, `${game.cn} ${primaryVisualCue.resource} 对应真实图片`)}
                <figcaption>
                  <span>${t("articleVisualLook")}</span>
                  ${entityTitleMarkup(guide.game, primaryVisualCue.resource)}
                  ${primaryVisualCue.caption ? `<em>${primaryVisualCue.caption}</em>` : ""}
                  <p>${primaryVisualCue.text}</p>
                  ${visualSourceMarkup(primaryVisualCue)}
                </figcaption>
              </figure>
              <div class="visual-cue-list">
                ${visual.cues
                  .map(
                    (cue, index) => `
                      <article class="${cue.hasVisual ? "" : "is-missing-visual"}" style="--shot-pos: ${cue.position}">
                        ${visualMediaMarkup(cue, `${game.cn} ${cue.resource} 对应真实图片`)}
                        <div>
                          <span>${String(index + 1).padStart(2, "0")}</span>
                          ${entityTitleMarkup(guide.game, cue.resource)}
                          ${cue.caption ? `<em>${cue.caption}</em>` : ""}
                          <p>${cue.text}</p>
                          ${visualSourceMarkup(cue)}
                        </div>
                      </article>
                    `,
                  )
          ...
          <section id="article-prep" class="article-block">
            ...
            <div class="visual-resource-grid">
              ${article.prep
                .map((item, index) => {
                  const cue = visual.cues[index % visual.cues.length] || {};
                  return `
                    <article class="${cue.hasVisual ? "" : "is-missing-visual"}" style="--shot-pos: ${cue.position || "50% 50%"}">
                      ${visualMediaMarkup(cue, `${game.cn} ${guide.resources[index]} 资源检查图`)}
                      <div>
                        <span>${t("articleVisualResources")} ${String(index + 1).padStart(2, "0")}</span>
                        ${entityTitleMarkup(guide.game, guide.resources[index])}
                        ${cue.caption ? `<em>${cue.caption}</em>` : ""}
                        <p>${item}</p>
                        ${visualSourceMarkup(cue)}
                      </div>
            ...
            <div class="phase-list article-phase-list">
              ${visual.routePanels
                .map(
                  (phase) => `
                    <article>
                      <figure class="phase-shot ${phase.visual && phase.visual.hasVisual ? "" : "is-missing-visual"}" style="--shot-pos: ${phase.position}">
                        ${visualMediaMarkup(phase.visual, `${game.cn} ${phase.title} 术语对应图`)}
                        <figcaption>${t("articleVisualRoute")} ${phase.label}</figcaption>
                      </figure>
                      <div>
          ...
          <section id="article-terms" class="article-block">
            ...
            <div class="term-grid">
              ${terms
                .map((item) => {
                  const entity = entityForGameTerm(guide.game, item.term);
                  if (!entity) {
                    return `
                      <article>
                  ...
                  return `
                    <a class="term-card" href="${entityPath(entity)}" data-entity="${entity.id}">
                      <strong>${item.term}</strong>
                      <p>${item.description}</p>
                      <span>${t("databaseOpen")}</span>
                    </a>
                  `;
                })
                .join("")}
                ...
                .map(
                  (item) => `
                    <a href="${guidePath(item.id)}" data-guide="${item.id}" class="related-guide article-card">
                      <img src="${item.image}" alt="${games[item.game].cn} ${item.title} 相关攻略图" loading="lazy">
...
function selectDatabase(id, updateHistory = true) {
  if (!games[id]) return;
  state.selectedGame = id;
  state.game = id;
  state.selectedGuide = null;
  state.selectedDatabaseGame = id;
  state.selectedEntity = null;
  if (updateHistory && !updateRoute({ type: "database", id }, databasePath(id))) return;
  render();
  $("#games").hidden = false;
  $("#games").scrollIntoView({ behavior: "smooth", block: "start" });
}

function selectEntity(id, updateHistory = true) {
  ...
  state.selectedDatabaseGame = null;
  state.selectedEntity = entity.id;
  if (updateHistory && !updateRoute({ type: "entity", id }, entityPath(entity))) return;
  render();
  $("#games").hidden = false;
  $("#games").scrollIntoView({ behavior: "smooth", block: "start" });
}
...
function bindControls() {
  document.addEventListener("click", (event) => {
    const entityLink = event.target.closest("[data-entity]");
    if (entityLink && entityLink.dataset.entity) {
      event.preventDefault();
      selectEntity(entityLink.dataset.entity);
      return;
    }

    const databaseLink = event.target.closest(".database-link[data-database-game]");
    if (databaseLink && databaseLink.dataset.databaseGame) {
      event.preventDefault();
      selectDatabase(databaseLink.dataset.databaseGame);
      return;
    }
